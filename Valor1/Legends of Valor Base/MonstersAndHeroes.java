...
public class MonstersAndHeroes extends Game {
	...
	private World map; //the game map
	...
	public MonstersAndHeroes() throws Exception {
		...
		//reading heroes
		Paladin[] paladins = Factory.createPaladins(".\\Paladins.txt");
		Sorcerer[] sorcerers = Factory.createSorcerers(".\\Sorcerers.txt");
		Warrior[] warriors = Factory.createWarriors(".\\Warriors.txt");
		int index = 0;
		for (int i=0; i<paladins.length; i++) {
			heroes[index] = paladins[i];
			index++;
		}
		for (int i=0; i<sorcerers.length; i++) {
			heroes[index] = sorcerers[i];
			index++;
		}
		for (int i=0; i<warriors.length; i++) {
			heroes[index] = warriors[i];
			index++;
		}
		heroNum = index;
		
		//reading monsters
		Dragon[] dragons = Factory.createDragons(".\\Dragons.txt");
		Exoskeleton[] exoskeletons = Factory.createExoskeletons(".\\Exoskeletons.txt");
		Spirit[] spirits = Factory.createSpirits(".\\Spirits.txt");
		index = 0;
		for (int i=0; i<dragons.length; i++) {
			monsters[index] = dragons[i];
			index++;
		}
		for (int i=0; i<exoskeletons.length; i++) {
			monsters[index] = exoskeletons[i];
			index++;
		}
		for (int i=0; i<spirits.length; i++) {
			monsters[index] = spirits[i];
			index++;
		}
		monsterNum = index;
		...
		index = 0;
		//reading items
		Armor[] armors = Factory.createArmors(".\\Armory.txt");
		FireSpell[] fireSpells = Factory.createFireSpells(".\\FireSpells.txt");
		IceSpell[] iceSpells = Factory.createIceSpells(".\\IceSpells.txt");
		LightningSpell[] lightningSpells = Factory.createLightningSpells(".\\LightningSpells.txt");
		Potion[] potions = Factory.createPotions(".\\Potions.txt");
		Weapon[] weapons = Factory.createWeapons(".\\Weaponry.txt");
		index = 0;
		for (int i=0; i<armors.length; i++) {
			items[index] = armors[i];
			index++;
		}
		for (int i=0; i<fireSpells.length; i++) {
			items[index] = fireSpells[i];
			index++;
		}
		for (int i=0; i<iceSpells.length; i++) {
			items[index] = iceSpells[i];
			index++;
		}
		for (int i=0; i<lightningSpells.length; i++) {
			items[index] = lightningSpells[i];
			index++;
		}
		for (int i=0; i<potions.length; i++) {
			items[index] = potions[i];
			index++;
		}
		for (int i=0; i<weapons.length; i++) {
			items[index] = weapons[i];
			index++;
		}
		itemNum = index;
		...
		map = new World(width, width, 1, items, itemNum);
		currentRow = 0;
		currentCol = 0;
	...
	public void start() throws Exception {
		while (true) {
			...
			map.print();
			if (map.getSpace(currentRow, currentCol) instanceof MarketSpace) {
			...
			if (moved && map.getSpace(currentRow, currentCol) instanceof CommonSpace) {
				//battle or not
	...
	public boolean enterBattle() {
		...
		if (whoWon == Battle.HERO) {
			...
			for (int i=0; i<myParty.length; i++) {
				myParty[i].gainExp(Rule.expGain(highestLevel));
				myParty[i].gainGold(Rule.goldGain(highestLevel));
				myParty[i].reset();
			}
			for (int i=0; i<opponent.length; i++) {
				opponent[i].reset();
	...
	public void enterMarket() {
		MarketSpace currentSpace = (MarketSpace)map.getSpace(currentRow, currentCol);
